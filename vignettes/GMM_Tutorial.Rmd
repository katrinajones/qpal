...
Now try for yourself, using an example dataset of photos of Carnivoran tarsals from XXXX [cite], in the folder `2Dtarsals`. Locate this directory using `system.file("extdata", package="qpal")`. Associated data are located in `Tarsalsdata.csv`. To analyze example landmarks, there is a ready-made geomorph dataframe which can be accessed using `data("tarsalsdf")`.
...
dimnames(tarsalsdf$coords)[[3]]<-gsub(" ", "_", tarsalsdf$species)
...
ntraits<-12
simdat<-two.d.array(tarsalsdf$coords)[,1:ntraits]
rownames(simdat)<-gsub(" ", "_", tarsalsdf$species)
...
anc.st<-arrayspecs(phy.tar, 17, 2)
plot(anc.st[,,1])
...
names(tarsalsdf$family)<-gsub(" ", "_", tarsalsdf$species)
...
[Insert example with new landmarks]

##Practical - Analysis of 3D Tarsals
